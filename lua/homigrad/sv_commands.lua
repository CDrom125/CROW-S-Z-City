...
if SERVER then
    ...
    COMMANDS.whois = {function(ply)
        ...
        for _, p in player.Iterator() do
            if p.isTraitor then
                traitors[#traitors + 1] = p:Name()
            end
            if p.isGunner then
                gunners[#gunners + 1] = p:Name()
            end
        end

        if #traitors == 0 and #gunners == 0 then
            ply:ChatPrint("No roles assigned yet.")
            return
        end

        if #traitors > 0 then
            ply:ChatPrint("Traitor(s): " .. table.concat(traitors, ", "))
        end
        if #gunners > 0 then
            ply:ChatPrint("Sheriff(s): " .. table.concat(gunners, ", "))
        end
    end, 2}
